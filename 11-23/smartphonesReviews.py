...
if __name__ == '__main__':
    ...
    reviews, classification, corpus, sentences = readTagMessages(
    ...
    s = [sentence for sentence in sentences
         if aspects[0] in cleanText(sentence, True, 2, False).split()]
    # pprint(s)

    vectorizer = TfidfVectorizer(min_df=1, ngram_range=(1, 1))
    featureMatrix = vectorizer.fit_transform(s).astype(float)
    similarityMatrix = featureMatrix * featureMatrix.T

    similarityGraph = networkx.from_scipy_sparse_matrix(similarityMatrix)
    networkx.draw_networkx(similarityGraph)

    plt.subplot(111)
    scores = networkx.pagerank(similarityGraph)
    pprint(scores)
    scores = sorted(((score, index) for index, score in scores.items()),
                    reverse=True)
